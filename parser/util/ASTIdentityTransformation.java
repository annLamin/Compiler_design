...
public class ASTIdentityTransformation extends ASTVisitor {
  ...
  public Object visitDoWhileStatement(
		  ASTDoWhileStatement dow, Object indent) {
		  String s = indent + "do ";
		  if (dow.statement instanceof ASTBlockStatement) {
		  s += dow.statement.accept(this, indent) + " ";
		  s += " while (" + dow.expression.accept(this, "") + ")\n";
		  } else {
		  s += "\n" + dow.statement.accept(this, indent + " ");
		  s += indent + "while (" + dow.expression.accept(this, "") +
		  ");";
		  }
		  return s;
		  }

public String visitBreakStatement(ASTBreakStatement astBreakStatement, Object indent) {
    String s = indent + "break;\n";
	return s;
}

public String visitContinueStatement(ASTContinueStatement astContinueStatement, Object indent) {
	 String s = indent + "continue;\n";
		return s;
}
}
